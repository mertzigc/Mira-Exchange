...
// Vänlig "vi har tagit emot" – ingen intern CTA, ingen intern data.
// ────────────────────────────────────────────────────────────
async function tmplPublicRequestReceived(e, extra, toName, ctaLabel, item) {
  ...
  if (kind === "ticket") {
    const title = ent.Rubrik || ent.Title || "Din felanmälan";
    const descr = _firstThread(ent["Tråd"]) || ent.Beskrivning || "";
    const subject = item.subject_override || "Vi har tagit emot din felanmälan";
    const html = wrapLayout({
      toName, logoUrl: "", senderName, imageUrl: "", accent: (extra.accent_color || "#db6923"),
      tag: "Mottaget",
      headline: "Tack – felanmälan är registrerad",
      body: `<p style="font-size:14px;color:#c0c4d6;line-height:1.65;">`
          ...
          + (descr ? `<p style="font-size:13px;color:#8892aa;line-height:1.6;margin-top:10px;">`
                   + `<em>${esc(descr)}</em></p>` : ""),
      details: detailRows([
  ...
  // booking
  const title   = ent.commission_title || ent.Title || "Din förfrågan";
  ...
  const html = wrapLayout({
    ...
    body: `<p style="font-size:14px;color:#c0c4d6;line-height:1.65;">`
        ...
        + `och granskas av internservice${senderName ? ` hos ${esc(senderName)}` : ""} `
        + `innan den bekräftas. Vi hör av oss.</p>`
        + (descr ? `<p style="font-size:13px;color:#8892aa;line-height:1.6;margin-top:10px;">`
                 + `<em>${esc(descr)}</em></p>` : ""),
    details: detailRows([
      ...
      senderName         && ["Företag",       senderName],
      extra.office       && ["Kontor",        extra.office],
      ent.Category       && ["Kategori",      esc(ent.Category)],
      delivDt            && ["Önskat datum",  delivDt],
      ent.delivery_address && ["Plats",       esc(ent.delivery_address)],
      ent.Specialkost_fritext && ["Specialkost", esc(ent.Specialkost_fritext)]
    ]),
    ctaLabel: null, ctaUrl: null,
...
// "Ny förfrågan att granska" – full data + uppmaning att hantera i Mira.
// ────────────────────────────────────────────────────────────
async function tmplPublicRequestInternal(e, extra, toName, ctaLabel, item) {
  const { kind, ent } = await _fetchPublicEntity(item, extra);
  const senderName = extra.company_name || "";
  const submitter  = extra.submitter_name || "";

  if (kind === "ticket") {
    const title = ent.Rubrik || ent.Title || "Ny felanmälan";
    const descr = _firstThread(ent["Tråd"]) || ent.Beskrivning || "";
    const subject = item.subject_override || `Ny felanmälan (publik): ${title}`;
    const html = wrapLayout({
      toName, logoUrl: "", senderName, imageUrl: "", accent: (extra.accent_color || "#db6923"),
      tag: "Ny felanmälan · publik",
      headline: title,
      body: descr ? `<p style="font-size:14px;color:#c0c4d6;line-height:1.65;">${esc(descr)}</p>` : "",
      details: detailRows([
        senderName                  && ["Företag",          senderName],
        extra.office                && ["Kontor",           extra.office],
        submitter                   && ["Inrapporterat av", esc(submitter)],
        ent.reported_by_email       && ["E-post",           esc(ent.reported_by_email)],
        ent.reported_by_phone       && ["Telefon",          esc(ent.reported_by_phone)],
        ent.status                  && ["Status",           statusBadge(ent.status)],
        ["Källa", "Publik landningssida"]
      ]),
      ctaLabel: null, ctaUrl: null,
      miraNote: "Hantera felanmälan på Mira."
    });
    return { subject, html };
  }

  // booking
  const title   = ent.commission_title || ent.Title || "Ny beställning";
  const descr   = ent.Description || ent.description || "";
  const delivDt = fmtDateTime(ent.delivery_date || ent.DeliveryDate);
  const subject = item.subject_override || `Ny beställning (utkast): ${title}`;
  const html = wrapLayout({
    toName, logoUrl: "", senderName, imageUrl: "", accent: (extra.accent_color || "#db6923"),
    tag: "Beställning · utkast",
    headline: title,
    body: descr ? `<p style="font-size:14px;color:#c0c4d6;line-height:1.65;">${esc(descr)}</p>` : "",
    details: detailRows([
      senderName                  && ["Företag",       senderName],
      extra.office                && ["Kontor",        extra.office],
      submitter                   && ["Beställare",    esc(submitter)],
      ...
      ent.delivery_address        && ["Plats",         esc(ent.delivery_address)],
      ent.guest != null           && ["Antal",         String(ent.guest)],
      ent.Specialkost_fritext     && ["Specialkost",   esc(ent.Specialkost_fritext)],
      ent.Budget                  && ["Budget",        esc(String(ent.Budget))],
      ent.kostnadsstalle          && ["Kostnadsställe", esc(ent.kostnadsstalle)],
      ["Status", statusBadge(ent.commission_status || "Utkast")]
    ]),
    ctaLabel: null, ctaUrl: null,
    miraNote: "Granska och omvandla till beställning på Mira."
  });
  return { subject, html };
}
// ────────────────────────────────────────────────────────────
