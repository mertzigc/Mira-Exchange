...
**✅ DEAL-STATUS FRAMÅT vid signering + abonnemang 2026-08-13 (smoke, ej deployat):** Deal.Status flyttas nu framåt till "Avtal" när (a) en offert signeras (i `_checkAndCompleteRequest` efter Approved, deal=parent.deal) och (b) ett abonnemang skapas (`/admin/contracts/create` + `/import/commit`, deal=b.deal). FRAMÅT-BARA (nedgraderas aldrig) via ny testbar modul `deal_status.js` (`shouldAdvanceDealStatus`) + `_advanceDealStatus`-helper i index.js (best-effort, non-fatal). 22 smoke gröna. DEPLOY: index.js + deal_status.js (Render). Ingen Bubble-ändring.
