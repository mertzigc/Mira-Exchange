...
> Syfte: ny session ska kunna ta vid exakt här. Offert/signering/affär-vy i drift (se ⭐ STATUS); produktionsmodulen (kök) ej påbörjad.
>
...
## ⭐ STATUS 2026-08-07 — läget nu (LÄS FÖRST)

> Detaljerad, färsk logg finns i minnet `project-offert-produktion-fe.md` (läs den + denna). Denna sektion = snabb överblick. Fokus senaste sessionen: **samlade Affär-vyn** (inline edit, wizard-fixar, Tengella-order, manuell koppling, **rikare affärskort**).

**🔜 NÄSTA SPÅR (2026-08-13, planerat): FÖRETAGSLISTA + KUNDKORT native→render-HTML.** Bygg om Bubbles native företagslista + kundkort till render-baserade HTML-block (samma DI-mönster som affar_api/salj_api). Läs `reference-bubble-yesno-text` + `reference-bubble-hidden-input-strip` + `reference-bubble-data-api-keys` + `reference-bubble-multiblock-collision` INNAN. Källor: ClientCompany (5413 poster — använd DELADE `sharedCompanyMap`, hämta ALDRIG alla per request), Coworker (Kundföretag-ref), avtal/order/faktura per företag. Mönster att återanvända: kök-iPad-login (scoped token) om kundkortet ska bli kundvänt; affärskortets kedje-reverse-lookup; SWR-cachen.

**✅ SÄLJ v3.1: is_salesmanager="ja"-fix 2026-08-13 (deployat/verifierat):** Svensk app renderar yes/no-fält som **"ja"/"nej"** när det binds som text i HTML-block → `is_salesmanager` lästes alltid som false (läsvy trots salesmanager=ja). Fix i mira-motesbokning.html: `IS_MGR=/^(ja|yes|true|1|on|x)$/i`. Backend oberört (Data API ger riktig boolean). Se `reference-bubble-yesno-text`.

**✅ PRESTANDA: DELAD FÖRVÄRMD CC-CACHE + SWR 2026-08-13 (smoke + browser-verifierat, ej deployat):** Långsam laddning berodde på att ClientCompany (5413 poster) laddades sekventiellt (~55 Bubble-sidor) per modul. FIX: delad `sharedCompanyMap`/`sharedCompanyOwnerMap` i index.js (prewarm vid boot + bakgrundsrefresh var 10:e min, stale-while-revalidate, in-flight-dedup) injiceras i affär/sälj/produktion (fallback lokal → smokes orörda). + frontend SWR (sessionStorage) i mira-motesbokning.html (visar cache direkt, revalidering i bg, rensas vid egna mutationer). 7 shared_cache-smoke gröna. DEPLOY: index.js + affar_api.js + salj_api.js + produktion_api.js (Render) + mira-motesbokning.html (Bubble). Ingen Bubble-ändring för affär.

**✅ SÄLJ-MODUL: MÖTESTRATT + SÄLJMÅL + ATTRIBUTION 2026-08-13 (smoke + browser-verifierat, ej deployat):** Ersätter dashboardens native mötesbokningstratt. Nytt `salj_api.js` (+ /admin/salj i openPrefixes): `GET /admin/salj/moten` (Kundmöten per fas + attribution möte→affär via aktivitetens deal-fält + summering/konvertering), `GET /admin/salj/budget?month=` (säljare m. SalesBudget → mål + LIVE utfall), `POST /admin/salj/budget/set`. Nytt block `mira-motesbokning.html` (.mb-, 2 vyer: mötestratt + admin-säljmål med progress-barer/godkänn/lägg-till-säljare). Säljare = manuell lista (User m. SalesBudget). **v2 (2026-08-13):** mötesmål delas PER FAS (mal_fas1..mal_fas4/mal_ovrigt, total_kundmote=summa); roll-grindning: salesmanager ser alla + sätter mål, vanlig User ser bara sina egna read-only (blocket läser current_user+is_salesmanager; icke-chef→GET?user_id=ME; soft by_user-guard på set→403 om ej salesmanager). 32 smoke gröna + browser-verifierat chef+icke-chef. **⚠️ KRÄVER Bubble-fält: SalesBudget.total_affar + mal_fas1/mal_fas2/mal_fas3/mal_fas4/mal_ovrigt (number) + User.salesmanager (yes/no). Bind i blocket: current_user (Current User's unique id) + is_salesmanager (Current User's salesmanager).** **v3 (2026-08-13):** kundmöte redigerbart i tratten via expanderat kort (likt affärsvyn) — `POST /admin/salj/mote/:id/patch` med soft behörighet (ägare ELLER salesmanager, annars 403); alla kan expandera+läsa; Genomfört?→Mötesanteckning-fält. 40 smoke gröna. DEPLOY: index.js + salj_api.js (Render) + mira-motesbokning.html (NYTT block på startsidan).

**✅ DEAL-STATUS STEGVIS I TRATTEN 2026-08-13 (smoke, ej deployat):** Deal.Status flyttas nu framåt automatiskt: (a) offert SKICKAS för signering → "Offert" (`_createApprovalRequestInternal` efter OAR-create, grindat på payload.offert, deal från payload.deal el. härledd ur offertens deal-fält); (b) offert SIGNERAS → "Avtal" (`_checkAndCompleteRequest` efter Approved, deal=parent.deal); (c) abonnemang skapas → "Avtal" (`/admin/contracts/create` + `/import/commit`, deal=b.deal). Full tratt: Kundkontakt → Offert → Avtal. FRAMÅT-BARA (nedgraderas aldrig) via testbar modul `deal_status.js` (`shouldAdvanceDealStatus`) + `_advanceDealStatus`-helper (best-effort, non-fatal). 26 smoke gröna. DEPLOY: index.js + deal_status.js (Render). Ingen Bubble-ändring.

**✅ SKAPA AFFÄR FRÅN LEAD/AKTIVITET (inline) 2026-08-12 (smoke + browser-verifierat, ej deployat):** I expanderad lead/aktivitet-rad finns nu "＋ Skapa ny affär" bredvid koppla-widgeten (inline, ingen popup). Fält: Titel(obl)+Beskrivning+Kundföretag+Kategori+Prel.värde+Ägare (förifylls från raden); Status auto "Kundkontakt"; lead→status "Delegerad". Ny backend `POST /admin/affar/deal/create` (affar_api.js) → bubbleCreate("deal",...) + sätter källradens deal-fält + lead→Delegerad (best-effort). Frontend `createDealWidgetHtml`+`saveCreateDeal` i mira-affar-samlad.html (återanvänder fieldSearchCompanies/pickSugg + USERS). 24 create-smoke gröna + browser-verifierat (prefill titel/prel.värde/kundföretag/ägare-förval, save-payload korrekt, "Affär skapad & kopplad ✓"). **⚠️ VERIFIERA att "Delegerad" är giltigt i Lead:s status-OS** (annars droppas det tyst — deal skapas+kopplas ändå). DEPLOY: affar_api.js (Render) + mira-affar-samlad.html (Bubble).

**✅ ORDER-PDF-länk i order-vyn 2026-08-10 (smoke + browser-verifierat, ej deployat):** I produktionens **order-vy** (ej prep-listan) har varje orderrad nu "📄 Order-PDF" → laddar hem kund-Order-PDF:en (alla detaljer). Ny route `GET /admin/produktion/order/:id/pdf?kind=order|pm` (default order) i produktion_api → `renderOrderPdf` inwirat från offertEngine (index.js). Fungerar med köks-token (scoped) OCH admin-token. Länk + handler i BÅDE mira-produktion-ipad.html (kfetch) och mira-produktion.html (admin-token). 34 produktion-smoke gröna (+order-PDF route order/pm) + browser-verifierat (iPad: rätt endpoint, köks-token, öppnar PDF). DEPLOY: index.js + produktion_api.js (Render) + mira-produktion.html + mira-produktion-ipad.html (Bubble).

**✅ KÖKS-iPad-VY + LOGIN-GRIND 2026-08-10 (backend smoke + frontend browser-verifierat, ej deployat):** Ny publik sida (t.ex. mira-fm.com/produktion) där köken loggar in på iPad. Ny fil **`mira-produktion-ipad.html`** (kopia av mira-produktion.html, `.mpi-`-namnrymd) med: (a) **login-grind** (delad köks-kod), (b) **iPad-läge-toggle** (större/läsbart: checkbox 30×30px vs 17, tabellfont 16px, knappar 16px, default PÅ, persist localStorage), (c) **utloggning**. **SÄKERHET:** sidan bär ALDRIG PLANNING_ADMIN_TOKEN (den öppnar hela admin-ytan). Istället: `POST /admin/produktion/login {code}` → om rätt kod (env `KITCHEN_ACCESS_CODE`) mintas en **scoped, HMAC-signerad köks-token** (payload {scope:"produktion",exp}, 12h TTL, signerad med PLANNING_ADMIN_TOKEN server-side). Produktion-endpointsen accepterar nu admin-token ELLER köks-token (`_planningAuthed || _kitchenAuth.authed`) — köks-token ger BARA /admin/produktion/*, aldrig affär/offert/avtal. Auth-logik i **`kitchen_auth.js`** (isolerad, testbar): mint/authed(timing-safe HMAC+scope+exp)/verifyCode. Frontend: `x-kitchen-token` i localStorage, `kfetch` (401→login-grinden igen). 14 kitchen_auth-smoke gröna (round-trip, tamper, expiry, fel scope, cross-secret, 503 utan kod) + browser-verifierat (grind→fel/rätt kod→data med köks-token utan admin-token→iPad-toggle+persist→logout→401-återgång). **KRÄVER env `KITCHEN_ACCESS_CODE` i Render.** DEPLOY: index.js + kitchen_auth.js (Render) + mira-produktion-ipad.html (NYTT Bubble-block på egen sida, bind bara `api_host`, INGEN token). Intern mira-produktion.html oförändrad (kör vidare med admin-token).

**✅ MULTIDOK-BATCHEXPORT (produktion, tidsintervall → ETT PDF) 2026-08-10 (smoke + visuellt + frontend browser-verifierat, ej deployat):** "Exportera PDF"-knapp i produktionsvyn (respekterar dag/vecka/from–to) → `GET /admin/produktion/export?date=|from=&to=[&parts=list,prep,pm,order]`. Ett sammanslaget PDF med sidbrytningar, 4 sektioner (alla default): **(1) Leveranslista** (rad/order: datum/tid/order/kund/vår referens/antal/kök/status, sorterat på leverans_ts+tid), **(2) Aggregerad prep-lista per kök** (summerat kök→prep-kategori över ALLA ordrar + item-lista med ordernr/kund, Ej tilldelat rött sist), **(3) Kök-PM per order** (grupperat per kök), **(4) Kund-orderbekräftelser per order**. Backend i `offert_api.js`: builders refaktorerade till `ORDER_STYLES`/`buildOrderBody` + `PM_STYLES`/`buildOrderPmBody` (enkeldok-utdata IDENTISK) så batch komponerar EN HTML (alla scoped `.o-`/`.pm-`/`.be-`-stilar i head). `renderBatchExport({from,to,date,parts})` queryar MiraOrder på leverans_ts + status∈{Bekräftad,I produktion,Levererad} + source=mira_fe, laddar rader/kund/ansvarig (var_referens→offert→deal→ägare), renderar via contractRenderEngine. Exponerat i offertEngine → wired i index.js → produktion_api route. 19 batch-smoke gröna (parts-filter, aggregering, tom period, route) + hela sviten 169 grön + PDF visuellt verifierat + export-knapp browser-verifierad (dag `?date=`, vecka `?from=&to=`, öppnar PDF). **DEPLOY:** `offert_api.js` + `produktion_api.js` + `index.js` (Render) + `mira-produktion.html` (Bubble). Inga nya Bubble-fält. **Framtid:** Excel-variant för prep-data (kräver ny xlsx-gen); ZIP med separata PDF:er.

**✅ PRODUKTION FEEDBACK-RUNDA 2 (#1/#5/#6) 2026-08-10 (backend smoke + produktionsvyn browser-verifierad, ej deployat):**
- **#1 Vår referens på order (redigerbar):** `MiraOrder.var_referens` (User). Order-editorn (affär) har nu **person-dropdown** (alla users, sorterat) i redigera-läge + **läsvy visar "Vår referens"** (namn, annars affärens ägare "(ägare)" i grått som default). `GET /admin/affar/order/:id` returnerar `users[]` + `var_referens`/`_name` + `var_referens_default`/`_name` (offert→deal→ägare-kedja); patch tar emot `var_referens`. Produktionsvyn (`produktion_api.js`) använder override → annars offert→deal→ägare (samma kedja) för `ansvarig` på både rad + order.
- **#5 Markera klar (produktion + order, speglat i affär):** `MiraOrderRad.producerad` (yes/no) + `MiraOrder.klar_for_leverans` (yes/no). Produktions **prep-lista**: checkbox per rad → `POST /admin/produktion/rad/:id/producerad` (optimistisk, ingen reload → accordion stannar; klar rad dimmas + genomstruken). Produktions **order-vy**: kolumn **Prod.** (`producerade/row_count`, grön vid fullt) + **"Klar f. lev."**-toggle → `POST /admin/produktion/order/:id/leveransklar`. Affär: order-editorns huvud har "Klar för leverans"-checkbox + grön badge i läsvy, och **order-liggaren visar grön "✓ klar f. lev."-badge** bredvid status (via `nOrderM.klar_for_leverans`).
- **#6 Ångra leverans (order stannar dimmad):** Dagsvyns query släpper nu in **Levererad** (STATUS_DISPLAY = Bekräftad/I produktion/Levererad). Levererade ordrar **stannar i order-vyn men dimmas** (opacity .5, grön status-pill) med **"↩ Ångra leverans"** → `POST /admin/produktion/order/:id/status {status:"I produktion"}`. Icke-levererade har "✓ Levererad"-knapp som förr.
- Smoke: produktion 32 grön (Levererad stannar, producerad/leveransklar-persist, override-ansvarig), order 26 grön (users-lista, default-ägare, var_referens+klar patch). Hela sviten 148 grön. Produktionsvyn browser-verifierad (prep-checkbox+dimning, order-vy progress/klar-toggle/Ångra, alla POST-endpoints rätt payload).
- **Kök-PM (`offert_api.js buildOrderPmHtml`)** visar nu **"Vår referens"** i leverans-bannern (mellan Leverans och Plats), upplöst via `_resolveOrderAnsvarig` (var_referens-override → annars offert→deal→ägare). Verifierat visuellt + render-smoke (12 grön, inkl override-fall).
- **Bubble-fält KLARA (Christian, 2026-08-10):** `MiraOrder.var_referens` (User), `MiraOrder.klar_for_leverans` (yes/no), `MiraOrderRad.producerad` (yes/no).
- **DEPLOY:** `affar_api.js` + `produktion_api.js` + `offert_api.js` (Render) + `mira-affar-samlad.html` + `mira-produktion.html` (Bubble). Hela sviten 150 grön.

**✅ RIKARE AFFÄRSKORT (2026-08-07, browser-verifierat, EJ deployat):** Affärskortet visar nu (a) **belopp/summa per kedjesteg** ("N st · Σ kr") + pipeline-summeringsrad i huvudet (Offert Σ / Order Σ / Fakturerat Σ) + "est. <value_brutto>"; (b) **klickbart steg → dok-lista** (källbadge, nummer, belopp, status, Visa-knapp, grön "KOPPLAD"-tag på P3-kopplade); (c) **"＋ Koppla befintligt dokument" från kortet** (typ + sök → koppla till denna affär). Backend `affar_api.js`: `linked`-tagging i `/deal/:id` (true=nådde kortet via P3 reverse på eget `deal`-fält; false=via Deals listfält/offert-kedja) + ny `GET /admin/affar/doc-search?type=&q=` (kandidatdok; MiraOrder linkable:false→kopplas via offert; FortnoxOrder(TENGELLA) exkluderad). Nyans: dok som bara sitter i Deals LISTFÄLT (utan eget `deal`-fält) visar inget deal_name i doc-search + linked=false. 27 smoke gröna (`affar_richcard_smoke.mjs`). **DEPLOY: `affar_api.js` (Render) + `mira-affar-samlad.html` (Bubble).**

**✅ SKAPA-SHORTCUTS Tier 1 (Aktivitet + Todo) 2026-08-07, browser-verifierat, EJ deployat:** Tar bort Bubble-native-popuparna för skapande. Generell "＋ Skapa"-rad överst (Aktivitet/Todo/Offert/Avtal) + kontext-knappar i affärskortet ("Skapa i affären:") som förifyller deal+kund. Aktivitet+Todo = inline-paneler i `.af-` (ej fixed-overlay). Backend: `bubbleCreate` inwirat + `POST /admin/affar/aktivitet/create` + `POST /admin/affar/todo/create` (Todo-fält bekräftade; Todo saknar deal-fält → koppling via `Deal.todo`-append). 15 create-smoke + 27 richcard gröna. **⚠️ CROSS-BLOCK-MODAL (offert/avtal ＋knappar i affär-vyn) SKROTAD 2026-08-08:** Fungerade ej live (multi-instans global-clobbering + Bubble dolda grupper). ＋Offert/＋Avtal-knapparna BORTTAGNA ur affär-blocket (både generell rad + kort-actions). **Aktivitet + Todo behålls** (native i .af-, funkar). Offert/avtal-filerna har vilande open-hook-kod (as_modal default av → standalone oförändrat).

**✅ PRODUKTIONSMODUL FAS 1 — DAGSVY PER KÖK 2026-08-10 (browser-verifierat, ej deployat):** Ny modul `produktion_api.js` + block `mira-produktion.html`. `GET /admin/produktion/dag?date=` läser MiraOrder date-bounded på numeriskt `leverans_ts` + status ∈ {Bekräftad, I produktion}, grupperar rader per kök→prep-kategori, aggregerar SUM(antal); `POST /admin/produktion/rad/:id/kok` flyttar rad→kök. Frontend: datumväljare + per-kök-kort med prep-lista (expanderbar till per-order-rader) + kök-fördelnings-select. Prep-kategorier: Frallor/Kall lunch/Varm lunch/Sallad/Wraps/Mingel/Buffé/Bröd/Konditori (order-editorns prep = dropdown nu). 2 Kök inlagda. 11 smoke gröna. DEPLOY: index.js + produktion_api.js (Render) + mira-affar-samlad.html + mira-produktion.html (NYTT Bubble-block på egen sida, bind token). **NÄSTA Fas 2:** PDF/Excel-export (prep-lista/kök + per-order plocklistor).

**✅ ORDER-PDF (kund) + KÖK-PM 2026-08-09 (browser-verifierat, ej deployat):** Order-editorn ombyggd till dokument-läsvy + penn-ikon (mindre plottrig). Två PDF-knappar i order-huvudet: **Order-PDF** (kund "Orderbekräftelse", offert-PDF-look + tydlig leverans-box) och **Kök-PM** (grupperat per kök, highlightad leverans + Intern instruktion-ruta, mat utan priser). Byggt i offert_api.js (`buildOrderHtml`/`buildOrderPmHtml`/`renderOrderPdf` via contractRenderEngine, exponerat i offertEngine) + affar_api `POST /admin/affar/order/:id/render-pdf?kind=order|pm` + `intern_instruktion` i order-editorn. 10 render-smoke gröna. **⚠️ KRÄVER Bubble-fält `intern_instruktion` (text) på MiraOrder.** DEPLOY: index.js + offert_api.js + affar_api.js (Render) + mira-affar-samlad.html (Bubble).

**✅ PRODUKTIONSMODUL FAS 1 — ORDER-REDIGERING inline i affär-vyn 2026-08-09 (browser-verifierat, ej deployat):** Mira-order-rader (source=mira_fe) är klickbara → inline-editor: huvud (leveransdatum/tid/status/betalning/momstyp/valuta/villkor) + rader (benämning/antal/enhet/á-pris/rabatt/moms/**kök-select**/prep/beskrivning) + lägg-till/ta-bort rad + server-omräknade totaler. Ändrat leveransdatum → uppdaterar `leverans_ts` på order OCH alla rader (produktion queryar på det). leveransadress=geo→read-only. Fortnox/Tengella-order = läskopior (ej redigerbara). Backend: `GET/POST /admin/affar/order/*` + `/koks` (affar_api.js, `bubbleDelete` inwirat). 19 smoke gröna. DEPLOY: index.js + affar_api.js (Render) + mira-affar-samlad.html (Bubble). **NÄSTA Fas 2:** produktions-dagsvy (GET /admin/produktion/dag date-bounded på leverans_ts, grupperad per kök) — kräver prep-kategori-värden (§10.5) + Kok-poster. Fas 3: PDF/Excel-export.

**✅ OFFERT→ORDER-KONVERTERING i affär-vyn 2026-08-08 (browser-verifierat, ej deployat):** Kollegor hittar Mira-offert i liggaren/affärskortet → "Konvertera →"-knapp skapar MiraOrder (idempotent, via befintliga convertOffertToOrder); konverterad offert visar "✓ Order FE-…". Backend: `POST /admin/affar/offert/:id/convert` (bara source=mira_fe) + orderstatus-berikning (order_id/order_nr) i feed/list/deal. Fortnox-offert = read-only (beslut: avvakta övergångshantering). 11 smoke gröna. DEPLOY: index.js + affar_api.js (Render) + mira-affar-samlad.html (Bubble).

**Offert+Avtal (Tier 2) KLAR + browser-verifierat:** open-hooks byggda på de befintliga blocken. Båda har `data-mira="as_modal"` (sätt "1" i affär-sidans instans). **Offert:** `.ao-mh` modal-host (default display:contents = standalone oförändrat; as_modal → fixed overlay) + `window.miraOffertModal.open({deal,clientcompany,clientcompany_nm})/close`; open uppdaterar modul-vars + newOffert() + parent-lift. **Avtal:** `DEAL_LIVE` (muterbar, ersätter frozen WIZ_DEAL_ID i payloaden) + `window.miraAvtalModal.open/close` (sätter deal+CLIENT, resetWizard→prefill, öppnar wizard-masken via `window.miraAvtalWiz`); `.ad-modal` döljer list-kroppen men behåller wizard-masken. Unika globaler, 0 `?.`/`??`, krock-kontrakt hållet.

**DEPLOY (Tier 1+2): Render:** `index.js` + `affar_api.js`. **Bubble:** `mira-affar-samlad.html` + `mira-offert-admin.html` + `mira-abonnemang-deal.html` (klistra om). **Bubble-setup Tier 2:** lägg mira-offert-admin.html + mira-abonnemang-deal.html som HTML-element PÅ affär-sidan (samma sida som samlad-vyn), sätt `as_modal`=1 + bind planning_token/api_host på båda. Då funkar ＋ Offert/＋ Avtal (generellt + från affärskortet).
...
- **✅ Tengella-order Fas 1+2 (2026-08-07) — RÄTTAR tidigare not:** Verifierat mot kod: Tengella-workorders skrivs till `TengellaWorkorder`+`UnifiedOrder` (INTE FortnoxOrder; `/sync/v2/tengella-workorder`-vägen skriver FortnoxOrder(TENGELLA) men bara i `mode:write`). BESLUT: **kanonisk källa = raw `TengellaWorkorder`**. affar_api hämtar TengellaWorkorder i feed+`/list` (order), taggade **tengella**, syns oavsett affär; FortnoxOrder(connection=Housekeeping) exkluderas i display (dedup). Belopp = Σ(Quantity×Price) ur `workorder_rows_json` (inget total_price-fält finns). **Radexpandering:** klick på workorder → mini-tabell Art.nr/Benämning/Antal/Á-pris/Summa (ur samma JSON, inga N+1). Visa/PDF avvaktas (ingen Fortnox-PDF för workorders).
- **✅ Inline edit lead/aktivitet/affär + wizard-fixar (2026-08-07):** Lead/Aktivitet/Affär-rader expanderbara+redigerbara inline (ersätter native-popups). Wizard "Avtal från mall" (mira-abonnemang-*) fixad på 4 buggar: (1) sök-dropdown strippade identitetsklass via `className=` → `classList`; (2) fixed-modal fångad av Bubble-förälders stacking → lyft positionerade föräldrar i openWiz; (3) parse-tids-`const IS_LIVE` frös MOCK innan async data-val-token → läs token lat + poll; (4) `mira-approval-archive.html` DELAR rot-klassen `.aa-wrap` → admin scopar nu till `.aa-wrap` med wizard-mask. Se [[reference-bubble-multiblock-collision]] + [[reference-bubble-hidden-input-strip]] + [[reference-bubble-data-api-keys]].
- **✅ P3 manuell koppling (2026-08-07):** "Koppla till affär" på ALLA understeg (lead/aktivitet/offert/avtal/order/faktura/workorder) → enhetligt `deal`-ref-fält. `POST /admin/affar/link {row_type,source,id,deal_id}` + `GET /admin/affar/deals?q=`. Affärskortets kedja reverse-look:ar nu alla typer på `deal` (deduped mot Deal-listfält) → manuellt kopplade dok tänds. MiraOrder kopplas via sin offert.

**Bubble-typer byggda av Christian:** Offert (+kind), OffertRad, Kok, MiraOrder (ordernr=TEXT, `kundforetag` UTAN ö), MiraOrderRad, ClientCompany (+customer_type/faktura_email/faktura_referens/Kundansvarig=User), OfferApprovalRequest (+offert), Product (+default_kok), **Contract.deal (ref→Deal)**. Lead: `tilldelad`+`Kundansvarig`(User) fanns redan; nya `Lead.deal`+`Lead.todo`. Deal-listfält offert/order/invoice/lead/historik/kontaktpersoner/todo finns. **P3 deal-fält (2026-08-07, gemener `deal`, ref→Deal):** `FortnoxOffer.deal`, `FortnoxOrder.deal`, `FortnoxInvoice.deal`, `TengellaWorkorder.deal` (activitet_crm/Offert/Contract hade redan). activitet_crm: `genomfört`(yes/no) för Kundmöte-flödet.

**✅ INLINE EDIT lead+aktivitet+AFFÄR KLART 2026-08-07 (browser-verifierat):** Uppföljning efter första deploy: (a) **Lead-sökfälten fixade** — företag+todo gick via `/admin/planning/companies` (extra grindar) resp. osäker Todo-constraint → nu egna cache-baserade `GET /admin/affar/companies?q=` + `/admin/affar/todos?q=` (companyMap/todoMap-cache, samma auth som feed). (b) **Knappar tightade** — form max-width 1080, vänsterställd knapp m. avdelare+hover (var osynlig till höger i bred scroll-tabell). (c) **Affär-expansionen nu redigerbar** (skärmdump av native-popupen): kedje-stepper överst + redigerbara deal-fält under — Titel/Status(lead_status)/Sannolikhet(potential_affär OS 0–1, visas ×100 %)/Region(Öst/Väst/Syd/Nord)/Affärsvärde brutto+netto/Kategori(OS-LISTA: Food & Event/Housekeeping/Service & People/Other facility services, checkboxar)/Beskrivning + kopplingar Kundföretag(single)/Kontaktpersoner(chips, coworker-sök scope:at till kundföretag)/Todo(chips). `GET /admin/affar/deal/:id` utökad med `edit`-objekt; nya `POST /admin/affar/deal/:id/patch` + `GET /admin/affar/coworkers?company=&q=`. Deal-skrivnycklar (display-namn): `titel/beskrivning/Status/Region/sannolikhet/Kategori(array)/value_brutto/value_netto/kundföretag/kontaktpersoner(id-array)/todo(id-array)`. 62 smoke-assertions gröna + browser-verifierat (deal-editor, checkboxar, chip-add/remove, coworker-sök). **Deploy: `affar_api.js` (Render) + `mira-affar-samlad.html` (Bubble).**

**✅ INLINE EDIT lead+aktivitet (grund) 2026-08-07:** Lead- och Aktivitet-rader i samlade vyn är nu **expanderbara + redigerbara inline** (ersätter native-popuparna). Aktivitet: typ-select; vid **Kundmöte** visas Fas/Mötesdatum/Genomfört, vid Genomfört visas Mötesanteckning (villkorlig toggle verifierad båda håll). Lead: koppla Kundföretag (sök) + Kundansvarig (User-dropdown/kollegor) + Todo (titelsök) — manuell koppling då auto ofta felar. **Skrivnycklar skarpt bekräftade via round-trip:** display-namn = skriv-nyckel (`Datum_bokning`, `Kundmöte`, `genomfört`, `mötesantecking`, `beskrivning`, `activity_type`; Lead: `client_company`, `Kundansvarig`, `todo`). **Bonus-fix:** `nAktFull` läste fel nycklar (`datum_bokning_date`/`kundm_te_option_kundm_te`) → Fas/Mötesdatum-kolumnerna var tomma; nu `Datum_bokning`/`Kundmöte`. Backend: 3 nya routes i `affar_api.js` (`POST /admin/affar/aktivitet/:id/patch`, `POST /admin/affar/lead/:id/link`, `GET /admin/affar/todos?q=`). 33 smoke-assertions gröna + browser-verifierat med mockad fetch. **Deploy: `affar_api.js` (Render) + `mira-affar-samlad.html` (Bubble).**

**⚠️ ÖPPNA PUNKTER / NÄSTA:**
1. **Deploya senaste (2026-08-07):** **Render:** `affar_api.js`. **Bubble (klistra om + bind `data-val` token efter varje omklistring!):** `mira-affar-samlad.html` + `mira-abonnemang-admin.html`/`-kund.html`/`-deal.html`. (index.js/offert_api.js orörda denna session.)
2. **STÄDNING namnrymd:** `mira-approval-archive.html` delar `.aa-`-namnrymden med abonnemang-admin (funktionellt löst via wizard-mask-scoping, men CSS läcker). Döp om approval-archives `aa-`→`ar-` för full frikoppling.
3. **Tengella-order-PDF (Fas 2b):** workorders saknar Fortnox-PDF → ingen Visa-knapp idag. Bygg enrich om Tengella-API ger workorder-PDF-URL (som fakturornas `Url`/`PdfUrl`) + fyll `ft_pdf`.
4. **Connection-auth-hälsa:** lazy-PDF (offert/order/faktura Fortnox) kan ge 2000311/2001101 på icke-F&E-anslutningar (token/refresh/licens). Ni har flera anslutningar (F&E `1771579463578x385222043661358460`, Housekeeping/Tengella `1771579481117x119544302020443410`, m.fl.).
5. **doc-typ-paginering approximativ** för offert/order (multi-källa: per-källa cursor+merge+slice). Enkla typer (lead/aktivitet/faktura/avtal/affär) har ren cursor-paginering. Order inkluderar nu även TengellaWorkorder-källan.
6. **"Mina offerter"-lista** placering; uppladdad offert saknar strukturerat belopp (total=0).
7. **🔴 SÄKERHET:** `ANTHROPIC_API_KEY` + `PLANNING_ADMIN_TOKEN` exponerade i klartext flera ggr — ej roterade. Rotera.

**✅ STÄNGT denna session:** inline edit lead/aktivitet/affär · wizard-buggar (klassstrip/z-index/async-token/delad-aa-wrap) · Tengella-order Fas 1+2 · P3 manuell koppling · inferens-fält bekräftade · sökfält-fix · knapp-polish. Bubble-uppsättning kvar från tidigare (om ej gjort): `data-mira="current_user_company"` i affär-blocket; affärs-popupens Avtal-flik = bara deal-blocket.

**Nyckelfiler:** `offert_api.js` (offert+signering+convert), `affar_api.js` (feed + `/deal/:id`-kedja[reverse-lookup alla typer på `deal`] + `/list` + assign + inline-edit[`/aktivitet/:id/patch`, `/lead/:id/link`, `/deal/:id/patch`] + sök[`/companies`,`/todos`,`/coworkers`,`/deals`] + P3[`/link`]; cache-lager companyMap/userMap/dealMap/supplierMap/todoMap/companyOwnerMap; normaliserare n* inkl `nWorkorder`+`_woRows`), `index.js` (`_createContractsFromApprovalRequest` ~16500, `_createApprovalRequestInternal` ~16850, SERVICES-konstanter+CT_DEAL ~19720, affar-registrering + `/admin/affar/doc-url` ~19833, fetchAndStore*Pdf 3637/3716/14849, `fortnoxGetBinary` 3467 global client_secret, `ensureFortnoxAccessToken` 1621), `contract_render.js`/`pdf_utils.js`. HTML: `mira-offert-admin.html`, `mira-affar-samlad.html`, `mira-abonnemang-kund/-admin/-deal.html` (deal=namnrymds-klon, portas manuellt vid ändringar), `mira-approval-create.html`. Smoke-tester i scratchpad.
