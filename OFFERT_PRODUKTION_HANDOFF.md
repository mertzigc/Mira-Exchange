...
- **Affär = ryggrad + Deal↔Avtal-koppling (BYGGT + smoke-testat 2026-08-02, EJ deployat):** Struktur låst — både Offert och Avtal skapas inifrån en affär; Avtal återanvänder mall-wizarden. Christian byggde `Contract.deal` (single ref→Deal). Klart i kod: (1) auto-create sätter `Contract.deal` från OAR.deal; (2) affär-kedjans deal-endpoint reverse-lookup:ar Contract → `chain.avtal`; (3) affärskortet visar Avtal-steg (mellan Offert och Order); (4) kund-wizarden (`mira-abonnemang-kund.html`) läser `data-mira="deal"` + skickar det till render-and-send. Backend-kedjan komplett end-to-end. **KVAR (Christian, Bubble):** bädda kund-wizarden i affärs-popupen med `data-mira="deal"` + "Skapa ny ▸ Offert/Avtal"-väljare. Globala "Alla avtal"-wizarden lämnas deal-lös per beslut. Legacy deal-lösa Contracts → P3 manuell koppla.
...
1. **Deploya senaste** — `index.js`, `offert_api.js`, `affar_api.js`, `mira-offert-admin.html`, `mira-affar-samlad.html`, `mira-abonnemang-kund.html`. (Uppladdad-signering-fixen + Affär-ryggrad-kopplingen 2026-08-02 måste ut.)
