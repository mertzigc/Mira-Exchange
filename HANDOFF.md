...
- **`companies_smoke.mjs`** — 128/128 gröna. `index.js` — wiring + delade cachar + Bubble-wf-callers. `emailer.js` — mallar `password_reset` + `user_welcome`.
...
- **⚠️ RUMS-HÄMTNING (fix 2026-08-15 efter deploy):** native-skapade rum saknar ofta tillbaka-ref — **interna lokaler ligger BARA i `Office.intern_lokal`-listan (kontor-fältet tomt); mötesrum har dock `office` satt.** Första versionen queryade bara på ref → interna lokaler visades tomma trots att native visade dem. Fix: `_officeRooms(office, oid)` hämtar via BÅDA vägar — per-id ur Office-listan (`Mötesrum`/`intern_lokal`) **+** ref-query (`office`/`kontor`) — union+dedup. (Rums-antal/kontor litet → per-id bubbleGet ok.) Nya rum vi skapar sätter både ref OCH append → fångas ändå.
