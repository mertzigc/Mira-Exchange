...
const NIGHTLY_INTERNAL_TIMEOUT_MS =
  Number(process.env.NIGHTLY_INTERNAL_TIMEOUT_MS || (15 * 60 * 1000)); // 15 min default
// ────────────────────────────────────────────────────────────
...
app.get("/fortnox/authorize", (req, res) => {
  ...
  const FORTNOX_SCOPE = String(
    process.env.FORTNOX_SCOPE ||
      "customer order offer invoice article"
  )
    .trim()
    .replace(/\s+/g, " ");
...
// - page, limit
// - lastmodified (optional)  e.g. "2026-03-05 09:45" (UTC recommended)
// - enrich_detail: true/false  (default true)   <-- frikopplad från PDF!
// - fetch_pdf: true/false (default false)
// - pdf_missing_only: true/false (default true)
// - pdf_max_per_page: throttle (default 10)    <-- används även som throttle för detail
// - pdf_pause_ms: throttle (default 400)
app.post("/fortnox/upsert/offers", async (req, res) => {
  try {
    ...
    // 2) Token behövs för detail och/eller pdf
    const wantPdf = boolish(fetch_pdf, false);
    const wantDetail = boolish(enrich_detail, true);

    let tok = null;
    if (wantPdf || wantDetail) {
    ...
    for (const o of offers) {
      ...
      try {
        // Hitta befintlig (behövs för pdf-flag + update/create)
        const existing = await bubbleFindOne("FortnoxOffer", [
          { key: "connection", constraint_type: "equals", value: connection_id },
          { key: "ft_document_number", constraint_type: "equals", value: docNo }
        ]);

        const existingHasPdf = !!(existing?.ft_pdf || existing?.item?.ft_pdf);

        // Bas-payload från LIST (kan sakna YourReferenceNumber)
        const payload = {
          connection: connection_id,
          ft_document_number: docNo,
          ...
          ft_url: String(o?.["@url"] || ""),
          ft_raw_json: JSON.stringify(o || {}),
          // Rows sync vill vi alltid flagga på list-sync (rows kommer separat)
          needs_rows_sync: true,
          // PDF-flag: endast relevant om vi kör PDF i detta pass (annars rör vi inte flaggan)
          ...(wantPdf ? { needs_pdf_sync: (missingOnly ? !existingHasPdf : true) } : {})
        };
        ...
        // 4) Detail-enrich (frikopplat från PDF)
        if (wantDetail) {
          const missingRef = !String(payload.ft_your_reference || "").trim();
          const withinThrottle = detail_enriched < maxPerPage;

          if (missingRef && withinThrottle) {
            ...
            if (det?.ok && det.offer) {
              ...
              detail_errors++;
              if (!firstDetailError) {
                firstDetailError = { ok: false, stage: "offer_detail_failed", docNo, detail: det };
              }
            }
          }
        }

        // 5) PDF fetch (valfritt)
        if (wantPdf) {
          const hasPdfNow = !!(existing?.ft_pdf || existing?.item?.ft_pdf);
          const shouldTryPdf =
            pdf_attempted < maxPerPage &&
            (!missingOnly || !hasPdfNow);
          ...
          if (shouldTryPdf) {
            ...
            try {
              ...
              if (pdfRes?.ok) {
                pdf_fetched++;
                // bränn flaggan när vi lyckas
                try { await bubblePatch("FortnoxOffer", bubbleId, { needs_pdf_sync: false }); } catch {}
              } else {
                pdf_errors++;
                if (!firstPdfError) {
                  firstPdfError = { ok: false, stage: "fetchAndStoreOfferPdf_failed", docNo, detail: pdfRes };
                }
              }
              ...
              pdf_errors++;
              if (!firstPdfError) {
                firstPdfError = { ok: false, stage: "fetchAndStoreOfferPdf_exception", docNo, detail: String(e?.message || e) };
              }
            }
          } else {
            pdf_skipped++;
    ...
    return res.json({
      ...
      meta,
      counts: { created, updated, skipped, errors },
      detail: { enabled: wantDetail, enriched: detail_enriched, errors: detail_errors, cfg: { maxPerPage } },
      pdf: { enabled: wantPdf, attempted: pdf_attempted, fetched: pdf_fetched, skipped: pdf_skipped, errors: pdf_errors, cfg: { missingOnly, maxPerPage, pauseMs } },
      first_error: firstError,
    ...
    console.error("[/fortnox/upsert/offers] fatal", e);
...
// ────────────────────────────────────────────────────────────
// /fortnox/upsert/offers/all  (delta only via lastmodified)
// Body:
// - connection_id (required)
// - days_back (default 7)  -> lastmodified = now - days_back
// - pages_per_call (default 2)  -> hur många pages vi tar per körning (snällt)
// - limit (default 100)
// - fetch_pdf (default false)  -> nightly bör köra false
app.post("/fortnox/upsert/offers/all", requireApiKey, async (req, res) => {
  try {
    const {
      connection_id,
      days_back = 7,
    ...
    const db = Math.max(1, Number(days_back) || 7);
    const now = new Date();
    const from = new Date(now);
    from.setUTCDate(from.getUTCDate() - db);

    // Fortnox lastmodified brukar acceptera "YYYY-MM-DD HH:mm"
    const pad2 = (n) => String(n).padStart(2, "0");
    const lm = `${from.getUTCFullYear()}-${pad2(from.getUTCMonth()+1)}-${pad2(from.getUTCDate())} ${pad2(from.getUTCHours())}:${pad2(from.getUTCMinutes())}`;

    const ppc = Math.max(1, Number(pages_per_call) || 2);

    const totals = { created: 0, updated: 0, skipped: 0, errors: 0 };
    let lastMeta = null;

    for (let page = 1; page <= ppc; page++) {
      ...
      // Om Fortnox säger @TotalPages=0 eller vi får 0 offers: bryt tidigt
      const tp = Number(j?.meta?.["@TotalPages"] || 0);
      const tr = Number(j?.meta?.["@TotalResources"] || 0);
      if (!tp || !tr) break;

      // snäll paus mellan sidor
      await sleep(250);
    }
...
// ────────────────────────────────────────────────────────────
// Caspeco – Hämta detail för en bokning (innehåller customer.orgNr + articles)
// List-endpointen returnerar customer=null; detail-endpointen ger fullt objekt.
// ────────────────────────────────────────────────────────────
async function caspecoFetchBookingDetail(guid) {
  try {
    return await caspecoFetch(`/WebBooking/WebBookings/${encodeURIComponent(guid)}`);
  } catch (e) {
    console.warn("[caspecoFetchBookingDetail] failed for", guid, e?.message);
    return null;
  }
}

// ────────────────────────────────────────────────────────────
// Caspeco – Säkerställ ClientCompany via customer.orgNr (bekräftat från detail-API)
// Skapar ClientCompany om det inte finns, precis som Fortnox/Tengella.
// ────────────────────────────────────────────────────────────
async function ensureClientCompanyForCaspecoBooking(detail) {
  const orgRaw = String(detail?.customer?.orgNr ?? "").trim();
  const orgNorm = normalizeOrgNo(orgRaw);
  if (!orgNorm) return null;

  const companyName = String(detail?.customer?.name ?? "").trim();
...
async function upsertFortnoxInvoiceDirect(connection_id, invoice) {
  ...
  // Deal-kedja: Er referens → annars Ert ordernummer
  const dealLink = yourReference || yourOrderNumber;

  const payload = {
    // ✅ rätt fält för invoice hos dig
    ...
    ft_ocr: String(invoice?.OCR || ""),

    ft_cancelled: invoice?.Cancelled === true,
    ft_sent: invoice?.Sent === true,
...
async function upsertFortnoxOrderRowsDirect(connection_id, order_docno) {
  const docNoReq = String(order_docno || "").trim();
  if (!connection_id || !docNoReq) {
    throw new Error("Missing connection_id or order_docno");
  }

  const tok = await ensureFortnoxAccessToken(connection_id);
  if (!tok?.ok || !tok?.access_token) {
    const e = new Error("ensureFortnoxAccessToken failed for order rows");
    e.detail = tok;
    throw e;
  }

  const r = await fortnoxGet(`/orders/${encodeURIComponent(docNoReq)}`, tok.access_token);
  if (!r?.ok) {
    const e = new Error("fortnoxGet order detail failed");
    e.detail = r;
    throw e;
  }
  ...
  const ordObj = Array.isArray(searchOrd) && searchOrd.length ? searchOrd[0] : null;
  if (!ordObj?._id) {
    throw new Error(`Parent FortnoxOrder not found in Bubble for ${ordDocNo}`);
  }
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      const existingId = existingByKey[uniqueKey] || null;
...
async function upsertFortnoxOfferRowsDirect(connection_id, offer_docno) {
  const docNo = String(offer_docno || "").trim();
  if (!connection_id || !docNo) {
    throw new Error("Missing connection_id or offer_docno");
  }

  const tok = await ensureFortnoxAccessToken(connection_id);
  if (!tok?.ok || !tok?.access_token) {
    const e = new Error("ensureFortnoxAccessToken failed for offer rows");
    e.detail = tok;
    throw e;
  }
  ...
  const r = await fortnoxGet(`/offers/${encodeURIComponent(docNo)}`, tok.access_token);
  if (!r?.ok) {
    const e = new Error("fortnoxGet offer detail failed");
    e.detail = r;
    throw e;
  }
  ...
  if (!parent?._id) {
    throw new Error(`Parent FortnoxOffer not found in Bubble for ${docNo}`);
  }
...
async function runFortnoxCronV1ForConnection({
  ...
  const summary = {
    ...
    days_back,
    windows: {
      customers_lastmodified: sinceStr,
  ...
  if (enable_order_rows) {
    const flaggedOrders = await bubbleFind("FortnoxOrder", {
      ...
      limit: Number(order_rows_limit) || 40
    });

    const orders = Array.isArray(flaggedOrders) ? flaggedOrders : [];
    summary.order_rows.parents = orders.length;
    ...
    for (const o of orders) {
      ...
      try {
        const rr = await upsertFortnoxOrderRowsDirect(connection_id, docNo);
        summary.order_rows.created += Number(rr?.counts?.created || 0);
        summary.order_rows.updated += Number(rr?.counts?.updated || 0);
        summary.order_rows.errors += Number(rr?.counts?.errors || 0);

        if (!summary.order_rows.first_error && rr?.first_error) {
          summary.order_rows.first_error = rr.first_error;
        }
      } catch (e) {
        summary.order_rows.errors++;
        if (!summary.order_rows.first_error) {
          summary.order_rows.first_error = {
            docNo,
            message: e?.message || String(e),
            detail: e?.detail || null
          };
  ...
  if (enable_offer_rows) {
    const flaggedOffers = await bubbleFind("FortnoxOffer", {
      ...
      limit: Number(offer_rows_limit) || 40
    });

    const offers = Array.isArray(flaggedOffers) ? flaggedOffers : [];
    summary.offer_rows.parents = offers.length;
    ...
    for (const o of offers) {
      ...
      try {
        const rr = await upsertFortnoxOfferRowsDirect(connection_id, docNo);
        summary.offer_rows.created += Number(rr?.counts?.created || 0);
        summary.offer_rows.updated += Number(rr?.counts?.updated || 0);
        summary.offer_rows.errors += Number(rr?.counts?.errors || 0);

        if (!summary.offer_rows.first_error && rr?.first_error) {
          summary.offer_rows.first_error = rr.first_error;
        }
      } catch (e) {
        summary.offer_rows.errors++;
        if (!summary.offer_rows.first_error) {
          summary.offer_rows.first_error = {
            docNo,
            message: e?.message || String(e),
            detail: e?.detail || null
          };
...
async function caspecoFetch(path, { method = "GET", query = null, body = null } = {}) {
  ...
  const url = new URL(`${CASPECO_BASE_URL}${path}`);
  url.searchParams.set("system", CASPECO_SYSTEM);
  ...
  const headers = {
    "Authorization": `Bearer ${CASPECO_PAT}`,
    "Content-Type": "application/json",
    "Accept": "application/json"
  };
  ...
  if (!res.ok) {
    const err = new Error(`Caspeco ${method} ${path} failed (${res.status})`);
    err.status = res.status;
    err.detail = { status: res.status, url: url.toString(), body: json };
    throw err;
  }
...
app.get("/caspeco/availability", requireApiKey, async (req, res) => {
  try {
    ...
    if (String(include_unavailability) === "true") query.includeUnavailability = true;
...
// ────────────────────────────────────────────────────────────
// GET /caspeco/booking/:id
// Hämtar en specifik bokning
// ────────────────────────────────────────────────────────────
app.get("/caspeco/booking/:id", requireApiKey, async (req, res) => {
  try {
    const { id } = req.params;
    if (!id) return res.status(400).json({ ok: false, error: "Missing booking id" });

    const data = await caspecoFetch(`/WebBooking/WebBookings/${encodeURIComponent(id)}`);

    return res.json({ ok: true, system: CASPECO_SYSTEM, booking: data });
  } catch (e) {
    return res.status(e.status || 500).json({
      ok: false,
      error: e?.message || String(e),
...
app.post("/caspeco/bookings/sync", requireApiKey, async (req, res) => {
  try {
    ...
    for (const b of list) {
      ...
      try {
        // Hämta detail för att få customer.orgNr + articles (section-info)
        // List-endpointen returnerar customer=null – detail krävs
        const detail = await caspecoFetchBookingDetail(bookingId);
        const bd = detail || b; // fallback till list-data om detail misslyckas
        ...
        const payload = {
          ...
          // ── Kontakt (Caspeco Contact-schema) ───────────────────
          contact_name:          String(
            (bd?.contact?.fname ? bd.contact.fname + " " + (bd?.contact?.lname ?? "") : "") ||
            bd?.internalContactName || ""
          ).trim(),
          ...
          // ── Raw JSON (detail-svaret om tillgängligt) ───────────
        ...
        errors++;
        if (!first_error) first_error = { bookingId, message: e?.message || String(e), detail: e?.detail || null };
      }
    }
...
// ClientCompany kopplas via ft_customer_number på ClientCompany (samma som Fortnox)
// ────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────
// Hämta och säkerställ kunddata för en Tengella-faktura:
// 1) Hämta TengellaCustomer från Bubble
// 2) Om org_no saknas → hämta kunden från Tengella API och upserta
// 3) Matcha/skapa ClientCompany via org_no
// 4) Sätt ft_customer_number på ClientCompany (samma mönster som Fortnox)
// Returnerar: { customerName, customerNumber, companyId }
// ────────────────────────────────────────────────────────────
async function resolveTengellaInvoiceCustomer(customerId, token) {
  ...
  // 2) Om org_no saknas → hämta från Tengella API (samma som workorders gör)
  if (tc && !tc.org_no && typeof tengellaGetCustomerById === "function") {
    try {
      ...
      console.warn("[tengella/invoices] customer fetch failed", custIdNum, e?.message);
    }
  }
  ...
  // 4) Matcha/skapa ClientCompany via org_no
  const orgRaw  = String(tc?.org_no || tc?.org_no_raw || tc?.organization_number || "").trim();
  const orgNorm = normalizeOrgNo(orgRaw);
  if (!orgNorm) return { customerName, customerNumber, companyId: null };
  ...
  return { customerName, customerNumber, companyId: ccId || null };
}

// Sätt ft_customer_number på ClientCompany om det saknas
async function ensureFtCustomerNumberOnClientCompany(ccId, custIdNum) {
  if (!ccId || !custIdNum) return;
  const cc = await bubbleFindOne("ClientCompany", [
    { key: "_id", constraint_type: "equals", value: ccId }
  ]).catch(() => null);
  if (!cc) return;
  if (cc.ft_customer_number === undefined || cc.ft_customer_number === null) {
    await bubblePatch("ClientCompany", ccId, { ft_customer_number: Number(custIdNum) }).catch(() => null);
  }
}
...
function mapTengellaInvoiceToFortnoxInvoicePayload(inv, { customerName = "", customerNumber = "" } = {}) {
  ...
  const totalValue   = inv?.TotalAmount != null ? String(inv.TotalAmount) : "";
  ...
  return {
    ...
    ft_customer_number:   customerNumber ? String(customerNumber) : "",
    ...
    ft_cancelled:         inv?.Void === true,
    ft_sent:              !!invoiceNo,

    ft_our_reference:     String(inv?.InvoiceType      ?? "").trim(),
    ft_your_order_number: "",
    ft_your_reference:    String(inv?.TaxReductionType ?? "").trim(),
...
// ────────────────────────────────────────────────────────────
async function upsertTengellaInvoiceToBubble(inv, { customerId = null, token = null } = {}) {
  const invoiceNo = String(inv?.InvoiceNo ?? "").trim() ||
                    String(inv?.InvoiceId  ?? "").trim();
  ...
  if (existingId) {
    ...
    return { ok: true, mode: "update", id: existingId, invoiceNo };
  }
  ...
  return { ok: true, mode: "create", id: createdId, invoiceNo };
}
    ...
    for (const customerId of customerIds) {
      ...
      while (more && page < maxPages) {
        ...
        for (const inv of data) {
          try {
            const r = await upsertTengellaInvoiceToBubble(inv, { customerId, token });
...
app.post("/analytics/articles", requireApiKey, async (req, res) => {
  ...
  try {
    ...
    const sortFns = {
      ...
      margin_pct: (a, b) => (b.margin_pct ?? -999) - (a.margin_pct ?? -999),
      margin_sek: (a, b) => (b.margin_sek ?? -999) - (a.margin_sek ?? -999)
    };
    ...
    return res.json({
      ...
      summary: {
        ...
        total_margin_sek: totMarginSek !== null ? Math.round(totMarginSek * 100) / 100 : null,
...
// ════════════════════════════════════════════════════════════════════════════
// ── ALLA fältnamn bekräftade av Christian mot Bubble-schemat (2026-06-16). EN sträng
// per fält, ingen hedging. Bubble är case-sensitivt; editorn versaliserar bara visuellt. ──
const FORFRAGAN = {
  // Typnamn
  ...
  OFFICE_COMPANY: "Kundföretag",

  // Erbjudande
  OFFER_COMPANY:  "Kundföretag",    // list of ClientCompany (tom = allmän; innehåller company = unik)
  OFFER_STATUS:   "Status",         // optionset "Status erbjudande": Utkast/Publicerat/Utgånget
  ...
  C_DELIVERY_END: "Slutdatum",
  C_CATEGORY: "Category",
  C_STATUS:   "commission_status",
  C_COMPANY:  "Company",
  C_DESC:     "Description",
  C_MESSAGE:  "commission_message",
  C_SUPPLIER: "Leverantör",         // list
  C_BUDGET:   "budget",
  C_OFFICE:   "Office",
  C_LOCATION: "location",
  C_GUEST:    "guest",
  ...
  // commission_status-värden (StatusUppdrag)
  STATUS_SENT:  "Ny",
  STATUS_DRAFT: "Utkast",

  // Underkategori-fält per kategori (bekräftat — inkonsekvent stavning i Bubble)
  SUBCAT_FIELDS: {
  ...
  NOTIFY_USER_EMAIL: "email",       // inbyggt Bubble-user-fält
  NOTIFY_EXTRA_INBOX: pick(process.env.FORFRAGAN_NOTIFY_EMAIL),

  // EmailQueue-fält (alla lowercase, bekräftat)
  EQ_TEMPLATE: "template_id", EQ_ENTITY_ID: "entity_id", EQ_ENTITY_TYPE: "entity_type",
  EQ_TO_EMAIL: "to_email", EQ_TO_NAME: "to_name", EQ_SENT: "email_sent", EQ_EXTRA: "extra_data",
};
...
// ── GET /admin/forfragan/schema — diagnostik: dumpar fältnycklar för en sample-rad ─
app.get("/admin/forfragan/schema", async (req, res) => {
  if (!_ffGuard(req, res)) return;
  try {
    const dump = async (type) => {
      try { const rows = await bubbleFind(type, { limit: 1 }); const s = rows?.[0]; return s ? { type, field_keys: Object.keys(s).sort(), sample: s } : { type, note: "tom typ" }; }
      catch (e) { return { type, error: e?.message }; }
    };
    return res.json({
      ok: true,
      offer:         await dump(FORFRAGAN.OFFER_TYPE),
      office:        await dump(FORFRAGAN.OFFICE_TYPE),
      clientcompany: await dump(FORFRAGAN.CLIENTCOMPANY_TYPE),
      comission:     await dump(FORFRAGAN.COMISSION_TYPE),
      lead:          await dump(FORFRAGAN.LEAD_TYPE),
      coworker:      await dump(FORFRAGAN.COWORKER_TYPE),
      user:          await dump(FORFRAGAN.USER_TYPE),
    });
  } catch (e) {
...
app.get("/admin/forfragan/bootstrap", async (req, res) => {
  ...
  try {
    ...
    let offices = [];
    try {
      const all = await bubbleFindAll(FORFRAGAN.OFFICE_TYPE, {});
      offices = all
        .filter((o) => !company || _ffIdOf(o[FORFRAGAN.OFFICE_COMPANY]) === company)
        .map((o) => {
          // office_address = geografisk adress (objekt {address,lat,lng}) → plocka strängen
          const raw = o[FORFRAGAN.OFFICE_ADDRESS];
          const address = raw == null ? "" : (typeof raw === "object" ? (raw.address || raw.formatted || "") : String(raw));
          return { id: bubbleId(o), name: o[FORFRAGAN.OFFICE_NAME] || "(namnlöst kontor)", address };
        });
    } catch (e) { offices = []; }
...
app.get("/admin/forfragan/offers", async (req, res) => {
  ...
  try {
    ...
    const unique  = company ? rows.filter((o) => o.company_ids.includes(company)) : [];
...
// ── GET /admin/forfragan/users?company=&q= — beställar-sök (COWORKER) ─────────
// Comission.Beställare = lista av Coworker (söks på e-post i Bubble). Därför söker vi
// Coworker (Kundföretag = company), inte User. Returnerar coworker-id (för Beställare +
// coworker-kedjan) + e-post (för lead) + namn.
app.get("/admin/forfragan/users", async (req, res) => {
  if (!_ffGuard(req, res)) return;
  try {
    const company = req.query.company ? String(req.query.company).trim() : null;
    const q = String(req.query.q || "").trim().toLowerCase();
    if (!company) return res.json({ ok: true, company_id: null, count: 0, results: [] });
    const all = await bubbleFindAll(FORFRAGAN.COWORKER_TYPE, {
      constraints: [{ key: FORFRAGAN.OFFICE_COMPANY, constraint_type: "equals", value: company }],
    });
    let rows = all.map((c) => ({
      id: bubbleId(c),
      email: c[FORFRAGAN.COWORKER_EMAIL] || "",
      fornamn: c["Förnamn"] || "",
      efternamn: c["Efternamn"] || "",
    })).filter((c) => c.email);
    if (q) rows = rows.filter((c) => (c.email + " " + c.fornamn + " " + c.efternamn).toLowerCase().includes(q));
  ...
  try {
    ...
    const buildComission = (startIso, isMaster) => {
      const exact = {
        ...
        [FORFRAGAN.C_DESC]:          rawMsg || null,
        [FORFRAGAN.C_MESSAGE]:       rawMsg || null,
        [FORFRAGAN.C_STATUS]:        statusValue,
        [FORFRAGAN.C_SOURCE]:        FORFRAGAN.SOURCE_VALUE,   // "Mira"
        [FORFRAGAN.C_COMPANY]:       companyId,
        [FORFRAGAN.C_INTERNSERVICE]: false,                    // yes/no → NO
        [FORFRAGAN.C_TITLE]:         title,
        [FORFRAGAN.C_CATEGORY]:      category || null,
        [FORFRAGAN.C_DELIVERY_END]:  endIso || null,
        [FORFRAGAN.C_BUDGET]:        d.budget != null ? asNumberOrNull(d.budget) : null,
        [FORFRAGAN.C_OFFICE]:        d.office_id ? String(d.office_id) : null,
        [FORFRAGAN.C_LOCATION]:      (d.plats || d.delivery_address) ? safeText(d.plats || d.delivery_address || "", 300) : null,
      ...
      if (rule) {
        exact[FORFRAGAN.C_REC_RULE]   = rule;
        exact[FORFRAGAN.C_REC_GROUP]  = recurrenceGroup;
        exact[FORFRAGAN.C_REC_MASTER] = isMaster;
        exact[FORFRAGAN.C_REC_UNTIL]  = untilIso || null;
      }
      ...
      return res.json({
        ...
        plan: {
          ...
          notify: { slug: FORFRAGAN.NOTIFY_SLUG, subject_preview: `${title} – ${subcatDisplay}`,
                    ...
                    recipient_count: (await _ffNotifyUsers(companyId)).map((u) => normEmail(u[FORFRAGAN.NOTIFY_USER_EMAIL])).filter(Boolean).length,
    ...
    for (let i = 0; i < occDates.length; i++) {
      const isMaster = i === 0;
      const { exact } = buildComission(occDates[i], isMaster);
      const payload = Object.fromEntries(Object.entries(exact).filter(([, v]) => v != null));   // droppa null
      const id = await bubbleCreate(FORFRAGAN.COMISSION_TYPE, payload);
      created.push(id);
    ...
    // notify (EmailQueue → emailer-poller). Users där Associated_company contains company.
    try {
    ...
    // Coworker.Bokningar (list of Comissions) += master per beställare
    for (const b of bestallare) {
      try {
        let cw = b.coworker_id ? await bubbleGet(FORFRAGAN.COWORKER_TYPE, String(b.coworker_id)).catch(() => null) : null;
        if (!cw && normEmail(b.email)) cw = await bubbleFindOne(FORFRAGAN.COWORKER_TYPE, [{ key: FORFRAGAN.COWORKER_EMAIL, constraint_type: "equals", value: normEmail(b.email) }]).catch(() => null);
        if (!cw) continue;
        const cur = Array.isArray(cw[FORFRAGAN.COWORKER_BOOKINGS]) ? cw[FORFRAGAN.COWORKER_BOOKINGS] : [];
        await bubblePatch(FORFRAGAN.COWORKER_TYPE, bubbleId(cw), { [FORFRAGAN.COWORKER_BOOKINGS]: cur.concat([masterId]) });
...
app.post("/admin/forfragan/update", async (req, res) => {
  ...
  try {
    ...
    const patch = {};
    if (d.commission_title != null) patch[FORFRAGAN.C_TITLE] = safeText(d.commission_title, 250);
    if (d.description != null) { patch[FORFRAGAN.C_DESC] = safeText(d.description, 5000); patch[FORFRAGAN.C_MESSAGE] = safeText(d.description, 5000); }
    ...
    if (d.subcategory && d.category && FORFRAGAN.SUBCAT_FIELDS[d.category]) patch[FORFRAGAN.SUBCAT_FIELDS[d.category]] = d.subcategory;

    if (!write) return res.json({ ok: true, mode: "diff", commission_id: id, patch });

    await bubblePatch(FORFRAGAN.COMISSION_TYPE, id, patch);
