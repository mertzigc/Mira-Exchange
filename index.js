...
app.post("/invoice/submit", async (req, res) => {
  ...
  try {
    ...
    console.log(`[invoice] Skapad InvoiceInquiry ${inquiryId} för ${company_name}` +
      (fortnoxInvoiceId ? ` | FortnoxInvoice: ${fortnoxInvoiceId}` : " | ingen FI-match") +
      (clientCompanyId  ? ` | CC: ${clientCompanyId}` : " | ingen CC-match")
    );

    // ── 6. Bilagor för SendGrid ───────────────────────────────────────────
    ...
    // ── 7. Notifiering till Carotte-teamet ────────────────────────────────
    ...
    <div style="font-family:Arial,sans-serif;font-size:14px;color:#333;max-width:600px">
      <h2 style="color:#db6923;margin-bottom:4px">Ny fakturafråga</h2>
      <p style="color:#666;margin-top:0">Mottagen via formuläret · Bubble ID: ${inquiryId}</p>
    ...
    await invSendgrid({
      ...
      replyTo: email,
      subject: `Fakturafråga – ${company_name} · ${case_type}`,
      html:    carotteHtml,
      attachments
    });

    // ── 8. Bekräftelse till kunden ────────────────────────────────────────
    const customerHtml = `
    <div style="font-family:Arial,sans-serif;font-size:14px;color:#333;max-width:600px">
    ...
    await invSendgrid({
      ...
      subject: "Bekräftelse: Din fakturafråga har mottagits",
      html:    customerHtml
    });
